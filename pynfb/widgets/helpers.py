...
def ch_names_to_2d_pos(list_of_ch_names, kind="standard_1005", azimuthal=True):
    ...
    indices = [
        upper_montage_ch_names.index(ch) if ch in upper_montage_ch_names else 0
        for ch in upper_list_of_ch_names
    ]
    if len(list(indices)) < len(list_of_ch_names):
        raise IndexError(
            "Channels {} not found".format(
                set(upper_list_of_ch_names).difference(
                    set(array(upper_montage_ch_names)[indices])
                )
            )
        )
    pos = (
        montage.pos[indices, :2]
        if not azimuthal
        else azimuthal_equidistant_projection(montage.pos[indices, :3])
    )
    return array(pos)
...
def project_to_2d(pos_3d, azimuthal=True):
    ...
    pos = (
        pos_3d[:, :2]
        if not azimuthal
        else azimuthal_equidistant_projection(pos_3d[:, :3])
    )
    return array(pos)
...
def validate_ch_names(list_of_ch_names, kind="standard_1005"):
    ...
    if DEBUG or seems_to_come_from_neuromag(list_of_ch_names):
        ...
        bool_indices = [
            ch in upper_montage_ch_names for ch in upper_list_of_ch_names
        ]
    return bool_indices
