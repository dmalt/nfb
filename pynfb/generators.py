...
from vendor.nfb.pynfb.io.brainvision import read_raw_brainvision
from vendor.nfb.pynfb.io.hdf5 import (
    load_h5py_all_samples,
    load_xml_str_from_hdf5_dataset,
    DatasetNotFound,
)
from vendor.nfb.pynfb.io.xml_ import get_lsl_info_from_xml
from vendor.nfb.pynfb.inlets.channels_selector import ChannelsSelector
...
def run_eeg_sim(
    freq=None, chunk_size=0, source_buffer=None, name="example", labels=None
):
    """
    ...
    if source_buffer is not None:
        source_buffer = np.concatenate(
            [source_buffer.T, source_buffer.T[::-1]]
        ).T
    ...
    while True:
        ...
        if source_buffer is not None:
            sample[: source_buffer.shape[0]] = source_buffer[
