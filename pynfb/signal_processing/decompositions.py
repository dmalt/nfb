...
DEFAULTS = {"bandpass_low": None, "regularizator": 0.05, "bandpass_high": None}
...
class SpatialDecomposition:
    ...
    def get_filter(self, index=None):
        ...
        filter_ = SpatialFilter(
            self.filters[:, index], self.topographies[:, index]
        )
        return filter_
...
class CSPDecomposition(SpatialDecomposition):
    ...
    def decompose(self, X, y=None):
        ...
        regularization = (
            lambda z: z
            + self.reg_coef * np.trace(z) * np.eye(z.shape[0]) / z.shape[0]
        )
...
class CSPDecompositionStimulus(CSPDecomposition):
    def __init__(
        ...
        super(CSPDecompositionStimulus, self).__init__(
            channel_names, fs, band, reg_coef
        )
        self.reg_coef = reg_coef
        self.name = "csp-s"
    ...
    def decompose(self, X, y=None):
        ...
        return super(CSPDecompositionStimulus, self).decompose(
            X[y >= 0], y[y >= 0]
        )
...
class ICADecomposition(SpatialDecomposition):
    def __init__(self, channel_names, channel_pos, fs, band=None):
        super(ICADecomposition, self).__init__(
    ...
    def decompose(self, X, y=None):
        raw_inst = RawArray(
            X.T, create_info(self.channel_names, self.fs, "eeg", None)
        )
    ...
    def get_scores(self, X, filters, ch_name=None, index=None):
        if index is None:
            index = (
                np.argmax(self.pos[:, 1])
                if ch_name is None
                else self.channel_names.index(ch_name)
            )
        self.sorted_channel_index = index
        components = np.dot(X, filters)
        scores = [
            mutual_info(components[:, j], X[:, index])
...
class SpatialDecompositionPool:
    def __init__(
        self, channel_names, fs, bands=None, dec_class="csp", indexes=None
    ):
        Decomposition = {"ica": ICADecomposition, "csp": CSPDecomposition}[
            dec_class
        ]
        ...
        self.pool = [
            Decomposition(channel_names, fs, band) for band in self.bands
        ]
    ...
    def get_filter(self):
        filters = [
            dec.filters[:, index]
            for dec, index in zip(self.pool, self.indexes)
        ]
        topographies = [
            dec.topographies[:, index]
            for dec, index in zip(self.pool, self.indexes)
        ]
...
if __name__ == "__main__":
    ...
    alpha = ButterFilter((8, 11), fs, 1).apply(
        np.random.normal(size=(n_samples, 1))
    )
    theta = ButterFilter((4, 7), fs, 1).apply(
        np.random.normal(size=(n_samples, 1))
    )
    beta = ButterFilter((17, 22), fs, 1).apply(
        np.random.normal(size=(n_samples, 1))
    )
    ...
    labels = np.append(
        np.zeros((sensors.shape[0] // 2)),
        np.ones((sensors.shape[0] // 2 + sensors.shape[0] % 2)),
    )
    csp_filter = (
        CSPDecomposition(["Pz", "Fp1", "C3"], fs)
        .fit(sensors, labels)
        .get_filter()
    )
