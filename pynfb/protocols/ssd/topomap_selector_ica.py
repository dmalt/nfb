...
from ...protocols.signals_manager.scored_components_table import (
    ScoredComponentsTable,
)
import numpy as np
...
from ...signal_processing.decompositions import (
    CSPDecomposition,
    ICADecomposition,
    CSPDecompositionStimulus,
)
...
class ICADialog(QtWidgets.QDialog):
    def __init__(
        ...
        if decomposition is None:
            if mode == "csp":
                if not _stimulus_split:
                    ...
                    self.decomposition = CSPDecompositionStimulus(
                        channel_pos, fs
                    )
                    labels = marks
            elif mode == "ica":
                self.decomposition = ICADecomposition(
                    channel_names, channel_pos, fs, band
                )
        else:
        ...
        self.table = ScoredComponentsTable(
            self.components,
            self.topographies,
            self.unmixing_matrix,
            channel_names,
            fs,
            self.scores,
            pos,
            scores_name=scores_name,
            marks=marks if _stimulus_split else None,
        )
        ...
        self.sliders = Sliders(
            fs, reg_coef=(mode == "csp"), stimulus_split=_stimulus_split
        )
        ...
        if mode == "ica":
            ...
            self.sort_combo.setCurrentIndex(
                self.decomposition.sorted_channel_index
            )
        ...
        # checkboxes behavior
        self.table.no_one_selected.connect(
            lambda: self.reject_button.setDisabled(True)
        )
        self.table.no_one_selected.connect(
            lambda: self.spatial_button.setDisabled(True)
        )
        self.table.one_selected.connect(
            lambda: self.reject_button.setDisabled(False)
        )
        self.table.one_selected.connect(
            lambda: self.spatial_button.setDisabled(False)
        )
        self.table.more_one_selected.connect(
            lambda: self.reject_button.setDisabled(False)
        )
        self.table.more_one_selected.connect(
            lambda: self.spatial_button.setDisabled(True)
    ...
    def sort_by_mutual(self):
        ...
        self.scores = [
            mutual_info(self.components[:, j], self.data[:, ind])
    ...
    def reject_and_close(self):
        ...
        self.rejection = SpatialRejection(
            np.dot(unmixing_matrix, inv),
            rank=len(indexes),
            type_str=self.mode,
    ...
    def recompute(self):
        ...
        self.bandpass = (
            parameters["bandpass_low"],
            parameters["bandpass_high"],
        )
    ...
    def get_rejection(
        ...
        wait_bar = WaitMessage(
            mode.upper() + WAIT_BAR_MESSAGES["CSP_ICA"]
        ).show_and_return()
        selector = cls(
            raw_data,
            channel_names,
            ...
            states=states,
            labels=labels,
            _stimulus_split=_stimulus_split,
            marks=marks,
        )
        wait_bar.close()
        ...
        bandpass = (
            selector.bandpass
            if selector.update_band_checkbox.isChecked()
            else None
        )
        return (
            selector.rejection,
...
if __name__ == "__main__":
    ...
    x = np.array(
        [
            np.sin(10 * (f + 1) * 2 * np.pi * np.arange(0, 10, 1 / fs))
    ...
    if False:
        ...
        with h5py.File(
            "{}\\{}\\{}".format(dir_, experiment, "experiment_data.h5")
        ) as f:
            ...
            x = np.concatenate(
                [x_dict["closed"], x_dict["opened"], x_dict["rotate"]]
            )
    ...
    for j in range(4):
        rejection, spatial, unmixing = ICADialog.get_rejection(
            x, channels, fs, mode="ica", labels=y
        )
