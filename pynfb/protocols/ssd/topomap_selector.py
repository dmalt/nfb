...
class TopomapSelector(QtWidgets.QWidget):
    def __init__(self, data, pos, names, sampling_freq=500, **kwargs):
        ...
        # topomap canvas
        self.topomaps = [
            TopographicMapCanvas(width=5, height=4, dpi=100)
    ...
    def draw_topomap(self):
        ...
        if not self.topomap_drawn[self.current_topomap]:
            self.topomap.update_figure(
                self.topographies[index][:, self.current_topomap],
                self.pos,
                names=self.names,
            )
    ...
    def get_current_bandpass(self):
        ...
        x2 = x1 + self.x_delta
        return (
            x1 - self.flanker_margin - self.flanker_delta,
            x2 + self.flanker_margin + self.flanker_delta,
        )
    ...
    def recompute(self):
        ...
        self.major_vals, self.topographies, self.filters = ssd_analysis(
            self.data,
    ...
    def underline_central_band(self):
        ...
        x1 = self.selector.current_x()
        x2 = x1 + self.x_delta
        self.selector.underline(
            x1 - self.flanker_margin - self.flanker_delta,
            x1 - self.flanker_margin,
            "flanker",
        )
        self.selector.underline(
            x2 + self.flanker_margin,
            x2 + self.flanker_margin + self.flanker_delta,
            "flanker",
        )
    ...
    def draw_lambda_level(self):
        self.selector.update_bin_level(
            self.x_delta,
            self.major_vals[
                self.selector.current_index(), self.current_topomap
            ],
        )
