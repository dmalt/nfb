...
class Montage:
    ...
    def __init__(self, ch_ordered_dict):
        """
        :param ch_dict: OrderedDict of Channel instances with ch. names as keys
        """
        assert (
            isinstance(ch_ordered_dict, OrderedDict)
            and len(ch_ordered_dict) != 0
            and all(
                isinstance(ch_ordered_dict[ch], Channel)
                for ch in ch_ordered_dict
            )
        )
    ...
    def pick_channels_by_type(self, ch_type):
        return np.nonzero(
            [self.ch_dict[ch].type == ch_type for ch in self.ch_dict]
        )[0]
    ...
    def plot_topography(self, data, types, axs=None):
        ...
        :param data:
            vector of numbers for each channel (including 'Aux' and similar)
        :param types:
            list of types of channels to plot.
            has to be a subset of types in the montage.
        :param axs:
            optional list of axes objects to plot into.
            if supplied should be the same lengths as types
        :return: list of axes objects it has plotted into
        """

        plottable_types = self.ch_types.difference([self.CH_TYPE_OTHER])
        assert plottable_types.issuperset(
            types
        ), "Can't plot channel types {}".format(
            str(set(types).difference(plottable_types))
        )
        assert len(types) != 0, "Supply at least one channel type for plotting"
        assert axs is None or len(types) == len(
            axs
        ), "Numbers of channel types and axes objects don't match"
...
if __name__ == "__main__":
    ...
    layout_grad = mne.channels.read_layout("Vectorview-grad")
    ch_list_from_lsl = (
        layout_1005.names + layout_mag.names + layout_grad.names + ["AUX1"]
    )
