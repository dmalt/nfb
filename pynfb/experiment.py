...
from .generators import (
    stream_file_in_a_thread,
    stream_generator_in_a_thread,
)
...
class Experiment:
    ...
    def update(self):
        ...
        chunk, other_chunk, timestamp = (
            self.stream.get_next_chunk()
        ...
        if chunk is not None and self.main is not None:
            ...
            sample = np.vstack(
                [np.array(signal.current_chunk) for signal in self.signals]
            ).T.tolist()
            ...
            if self.main.player_panel.start.isChecked():
                ...
                if self.samples_counter < self.experiment_n_samples:
                    ...
                    self.raw_recorder[chunk_slice] = chunk[
                        :, : self.n_channels
                    ]
                    ...
                    if self.activate_trouble_catching:
                        if self.samples_counter > self.seconds:
                            ...
                            raw_std_new = np.std(
                                self.raw_recorder[
                                    int(
                                        self.samples_counter - self.freq
                                    ) : self.samples_counter
                                ],
                                0,
                            )
                            if self.raw_std is None:
                                ...
                                if self.catch_channels_trouble and any(
                                    raw_std_new > 7 * self.raw_std
                                ):
                                    w = ChannelTroubleWarning(parent=self.main)
                                    w.pause_clicked.connect(
                                        self.handle_channels_trouble_pause
                                    )
                                    w.closed.connect(
                                ...
                                self.raw_std = (
                                    0.5 * raw_std_new + 0.5 * self.raw_std
                                )

            # redraw signals and raw data
            self.main.redraw_signals(
                sample,
                chunk,
                self.samples_counter,
                self.current_protocol_n_samples,
            )
            ...
            # redraw protocols
            is_half_time = (
                self.samples_counter >= self.current_protocol_n_samples // 2
            )
            ...
            if current_protocol.mock_previous > 0:
                samples = [
                    signal.current_chunk[-1]
                    for signal in current_protocol.mock
                ]
            elif current_protocol.mock_samples_file_path is not None:
                samples = self.mock_signals_buffer[
                    self.samples_counter % self.mock_signals_buffer.shape[0]
                ]
            else:
                samples = sample[-1]

            if (
                self.main.player_panel.start.isChecked()
                and self.samples_counter - chunk.shape[0]
                < self.experiment_n_samples
            ):
                self.reward_recorder[
                    self.samples_counter
                    - chunk.shape[0] : self.samples_counter
                ] = self.reward.get_score()

            if self.main.player_panel.start.isChecked():
                # subject update
                if self.params["bShowSubjectWindow"]:
                    mark = self.subject.update_protocol_state(
                        samples,
                        self.reward,
            ...
            if (
                self.samples_counter >= self.current_protocol_n_samples
    ...
    def next_protocol(self):
        ...
        # descale signals:
        signals_recordings = np.array(
            [
                signal.descale_recording(data)
                for signal, data in zip(
                    self.signals,
                    self.signals_recorder[: self.samples_counter].T,
                )
            ]
        ).T
        ...
        self.protocols_sequence[self.current_protocol_index].close_protocol(
            ...
            protocols_seq=[
                protocol.name
                for protocol in self.protocols_sequence[
        ...
        save_signals(
            ...
            protocol_name=self.protocols_sequence[
                self.current_protocol_index
            ].name,
            mock_previous=self.protocols_sequence[
                self.current_protocol_index
            ].mock_previous,
        ...
        self.samples_counter = 0
        if self.protocols_sequence[
            self.current_protocol_index
        ].update_statistics_in_the_end:
        ...
        if isinstance(
            self.protocols_sequence[self.current_protocol_index],
            FeedbackProtocol,
        ):
            if (
        ...
        if self.current_protocol_index < len(self.protocols_sequence) - 1:
            ...
            if current_protocol.mock_previous:
                ...
                if len(self.real_fb_number_list) > 0:
                    random_previos_fb = self.real_fb_number_list[
                        np.random.randint(0, len(self.real_fb_number_list))
                    ]
                if current_protocol.shuffle_mock_previous:
                    current_protocol.mock_previous = random_previos_fb
                if (
                    current_protocol.mock_previous
                    == self.current_protocol_index
                ):
                    ...
                    mock_raw = load_h5py(
                        self.dir_name + "experiment_data.h5",
                        "protocol{}/raw_data".format(
                            current_protocol.mock_previous
                        ),
                    )
                    mock_signals = load_h5py(
                        self.dir_name + "experiment_data.h5",
                        "protocol{}/signals_data".format(
                            current_protocol.mock_previous
                        ),
                    )

                current_protocol.prepare_raw_mock_if_necessary(
                    mock_raw, random_previos_fb, mock_signals
                )
            ...
            self.reward.signal = self.signals[
                reward_signal_id
            ]  # TODO: REward for MOCK
            self.reward.set_enabled(
                isinstance(current_protocol, FeedbackProtocol)
            )
    ...
    def restart(self):
        ...
        self.dir_name = "results/{}_{}/".format(
            self.params["sExperimentName"], timestamp_str
        )
        os.makedirs(self.dir_name)

        wait_bar = WaitMessage(
            WAIT_BAR_MESSAGES["EXPERIMENT_START"]
        ).show_and_return()
        ...
        events_stream = (
            LSLInlet(events_stream_name) if events_stream_name else None
        )
        ...
        # signals
        self.signals = [
            DerivedSignal.from_params(
                ind, self.freq, self.n_channels, channels_labels, signal
            )
            for ind, signal in enumerate(
                self.params["vSignals"]["DerivedSignal"]
            )
        ...
        self.composite_signals = [
            ...
            for ind, signal in enumerate(
                self.params["vSignals"]["CompositeSignal"]
            )
        ]
        ...
        for protocol in self.params["vProtocols"]:
            ...
            source_signal_id = (
                None
                if protocol["fbSource"] == "All"
                else signal_names.index(protocol["fbSource"])
            )
            reward_signal_id = (
                signal_names.index(protocol["sRewardSignal"])
                if protocol["sRewardSignal"] != ""
                else 0
            )
            mock_path = (
                protocol["sMockSignalFilePath"]
                if protocol["sMockSignalFilePath"] != ""
                else None,
            ...
            m_signal_index = (
                None
                if m_signal not in signal_names
                else signal_names.index(m_signal)
            )
            ...
            if protocol["sFb_type"] == "Baseline":
                self.protocols.append(
                    BaselineProtocol(
                        ...
                        if bool(protocol["bUseExtraMessage"])
                        else None,
                        **kwargs
                    )
                )
                ...
                raise TypeError(
                    'Undefined protocol type "{}"'.format(protocol["sFb_type"])
                )
        ...
        for name in self.params["vPSequence"]:
            ...
            if name in group_names:
                group = self.params["vPGroups"]["PGroup"][
                ...
                if group["bShuffle"]:
                    ...
                    subgroup = list(
                        subgroup[np.random.permutation(len(subgroup))]
                    )
                else:
                    subgroup = [
                        k
                        for k in chain(*zip_longest(*subgroup))
                        if k is not None
                    ]
        ...
        self.samples_counter = 0
        self.raw_recorder = (
            np.zeros((max_protocol_n_samples * 110 // 100, self.n_channels))
            * np.nan
        )
        self.timestamp_recorder = (
            np.zeros((max_protocol_n_samples * 110 // 100)) * np.nan
        )
        self.raw_recorder_other = (
            np.zeros(
                (max_protocol_n_samples * 110 // 100, self.n_channels_other)
            )
            * np.nan
        )
        self.signals_recorder = (
            np.zeros((max_protocol_n_samples * 110 // 100, len(self.signals)))
            * np.nan
        )
        self.reward_recorder = (
            np.zeros((max_protocol_n_samples * 110 // 100)) * np.nan
    ...
    def restart_lsl_from_file(self):
        ...
        self.thread = stream_file_in_a_thread(
            file_path, reference, stream_name
        )
