...
class SourceSpaceRecontructor(Protocol):
    def __init__(self, signals, **kwargs):
        ...
        self.mesh_data = self.get_mesh_data_from_inverse_operator(
            inverse_operator
        )
        self._forward_model_matrix = self._assemble_forward_model_matrix(
            inverse_operator
        )
    ...
    def _assemble_forward_model_matrix(inverse_operator):
        ...
        warn(
            "Currently info is read from the raw file."
            " TODO: change to getting it from the stream"
        )
        ...
        Id = np.identity(channel_cnt)
        dummy_raw = mne.io.RawArray(data=Id, info=info, verbose="ERROR")
        dummy_raw.set_eeg_reference(verbose="ERROR")

        # Applying inverse modelling to an identity matrix gives us the forward
        # model matrix
        snr = 1.0  # use smaller SNR for raw data
        lambda2 = 1.0 / snr ** 2
        method = "MNE"  # use sLORETA method (could also be MNE or dSPM)
        stc = mne.minimum_norm.apply_inverse_raw(
            dummy_raw, inverse_operator, lambda2, method
        )
        return stc.data
    ...
    def get_mesh_data_from_inverse_operator(self, inverse_operator):
        # Creates pyqtgraph.opengl.MeshData instance to store the cortical mesh
        # used to create the inverse operator.

        # mne's inverse operator is a dict with the geometry information under
        # the key 'src'.  inverse_operator['src'] is a list two items each of
        # which corresponds to one hemisphere.
        left_hemi, right_hemi = inverse_operator["src"]

        # Each hemisphere is represented by a dict that contains the list of
        # all vertices from the original mesh representaion of that
        # representation (with default options in FreeSurfer that is ~150K
        # vertices). These are stored under the key 'rr'.

        # Only a small subset of these vertices was most likely during the
        # construction of the inverse operator. The mesh containing only the
        # used vertices is represented by an array of faces sotred under the
        # 'use_tris' key.  This submesh still contains some extra vertices so
        # that it is still a manifold.

        # Each face is a row with the indices of the vertices of that face. The
        # indexing is into the 'rr' array containing all the vertices.

        # Let's now combine two meshes into one. Also save the indexes of the
        # used vertices into vertex_idx property
...
class SourceSpaceWidgetPainter(Painter):
    ...
    COLORMAP_BUFFER_LENGTH_DEFAULT = (
        40000
    )  # samples, if colormap limits set to 'global' then 'global' means last
    ...
    def __init__(
        self, source_space_reconstructor, show_reward=False, params=None
    ):
    ...
    def prepare_widget(self, widget):
        ...
        # We will only be assigning colors to a subset of vertexes used for
        # forward/inverse modelling. First, we need to
        ...
        # Set the camera at twice the size of the mesh along the widest
        # dimension
        max_ptp = max(np.ptp(self.cortex_mesh_data.vertexes(), axis=0))
        widget.setCameraPosition(distance=2 * max_ptp)

        self.cortex_mesh_item = gl.GLMeshItem(
            meshdata=self.cortex_mesh_data, shader="shaded"
        )
    ...
    def redraw_state(self, chunk):
        ...
        sources_normalized = self.normalize_to_01(
            last_sources, vmin=vmin, vmax=vmax
        )
